...
for i in range(len(os.listdir(test_directory))):
  for j, item in enumerate( os.listdir(os.path.join(test_directory,str(i))) ):
    ...
    if j < 5:
      image_result.append({
        "image": img_label,
        "predict": label_names[predict],
        "original": label_names[i]
      })
...
for i, result in enumerate(image_result):
    ...
    axes[i].set_title('Guess={}\nTrue={}'.format( result["predict"], result["original"]))
    axes[i].axis('off')
